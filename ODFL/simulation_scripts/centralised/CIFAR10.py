...
np.random.seed(42)
random.seed(42)
def launch_centralised_cifar10(
    lr: float,
    iterations: float,
    batch: int,
    device: torch.device,
    ...
    """
    Train a centralized CIFAR10 model with configurable optimizer and optional learning rate scheduler.

    This function sets up and runs a training loop for a centralized (non-federated) CIFAR10 model.
    ...
    dataset_directory = os.path.join(DATASET_PATH, 'centralised', 'CIFAR10')
    general_directory = os.path.join(RESULTS_PATH, 'centralised', 'CIFAR10', timestamp)
    results_directory = os.path.join(RESULTS_PATH, 'centralised', 'CIFAR10', timestamp, 'results')
    models_directory = os.path.join(RESULTS_PATH, 'centralised', 'CIFAR10', timestamp, 'models')
