...
np.random.seed(42)
random.seed(42)
def launch_centralised_pathmnist(
    lr: float,
    iterations: float,
    batch: int,
    device: torch.device,
    ...
    """
    Train a centralized PathMNIST model with configurable optimizer and optional learning rate scheduler.

    This function sets up and runs a training loop for a centralized (non-federated) PathMNIST model.
    ...
    dataset_directory = os.path.join(DATASET_PATH, 'centralised', 'PathMNIST')
    general_directory = os.path.join(RESULTS_PATH, 'centralised', 'PathMNIST', timestamp)
    results_directory = os.path.join(RESULTS_PATH, 'centralised', 'PathMNIST', timestamp, 'results')
    models_directory = os.path.join(RESULTS_PATH, 'centralised', 'PathMNIST', timestamp, 'models')
